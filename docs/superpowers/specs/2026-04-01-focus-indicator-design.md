...
- If managed → find all strip controllers whose strip contains at least one window owned by `pid` (using `tracker.windows` which carries per-window display info), and call `sc.showIndicator()` on each. In practice this is usually one display. If no match (race: app activated before windows are tracked), no-op. `showIndicator()` is a `StripController` wrapper that calls `updateFocusIndicator` (which routes through the bootstrap guard and snap-vs-animate heuristic). If the overlay was fully hidden (`currentFrame == nil`), this triggers a `snapTo` — the overlay appears at full opacity immediately (no fade-in for ring mode; flash mode uses its own flash animation on the next focus change).
